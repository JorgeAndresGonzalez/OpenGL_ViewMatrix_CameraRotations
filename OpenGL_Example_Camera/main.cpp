...
///Declare Flag for first mouse Input
float isFirstMouse = true;
float lastX = 400;
float lastY = 600;

///\////////////////////////////////////////////////////////////////////////////
///Declare the local view matrix
mat4 localMat;
...
///\/////////////////////////CAMERA CONTROLS////////////////////////////////////
    ...
    fCameraSpeed = 5.0f * deltaTime;

    ///Move front and back
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
    {
        vec3 cameraOffset = fCameraSpeed * camFront;
        camPos += cameraOffset;
    }
...
///\////////////////////////////////////////////////////////////////////////////
    ...
    {
        fYaw += fCameraSpeed * fSensitivity;
    ...
    {
        fPitch += fCameraSpeed * fSensitivity;
    ...
    ///Update new Roll angle
    if(glfwGetKey(window, GLFW_KEY_LEFT_ALT) == GLFW_PRESS)
    {
        fRoll -= fCameraSpeed * fSensitivity;
    ...
    if(camRotChanged)
    {
        ...
        ///Normally cameras only Yaw and Pitch
        if(!isCameraRollEnabled)
        {
            ///New CameraFront - Only Yaw
            /*
            newFront.x = cos(radians(fYaw));
            newFront.z = sin(radians(fYaw));
            newFront.y = 0;
            */
            ...
            */
            ...
            newRight = normalize(cross(newFront,worldUp));
        ...
        else
        {
            ///New Up
            newUp.z = 0;
            newUp.x = cos(radians(fRoll));
            newUp.y = sin(radians(fRoll));

            newUp = normalize(newUp);

            ///New Front - it doesn't change
            newFront = camFront;

            ///New Right
            newRight = normalize(cross(newFront,newUp));
        }
...
void setLocalMat(Shader s)
{
    ...
    ///uncomment this line if you are drawing the square
...
void setProjMat (Shader s)
{
    ...
    projMat = perspective(radians(45.0f), (float) fScreenWidth / (float)
                          fScreenHeight, 0.1f, 100.0f);
